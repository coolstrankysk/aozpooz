...
$(document).on('change', '#component-0-select', function () {
  substep = 2;
  selected = [];
...
$(document).on('change', '#component-1-select', function () {
  if (components[selected[0]]["components"][this.value]["components"]) {
    substep = 3;
    selected[1] = this.value;
    $('#component-2-select').remove();
    $('#component-3-select').remove();
    recalculateSteps();
  } else {
    showCriteria();
  }
...
$(document).on('change', '#component-2-select', function () {
  if (components[selected[0]]["components"][selected[1]]["components"][this.value]["components"]) {
    substep = 4;
...
$(document).on('change', '#component-3-select', function () {
  if (components[selected[0]]["components"][selected[1]]["components"][selected[2]]["components"][this.value]["components"]) {
    substep = 5;
...
$(document).on('click', '#new-deviation', function () {
  recordDeviation();
  refreshEvaluatedDeviations();
  step = 2;
  substep = 2;
  recalculateSteps();
  $('#scales').html('');
    ...
    case 2:
      ...
      switch (substep) {
        case 1:
          ...
          select.id = 'component-0-select';
          select.setAttribute('class', 'form-control form-group');
          $('#components').html(select);
          var option = document.createElement("option");
          option.value = null;
          option.text = "Vyberte";
          select.appendChild(option);
          $.each(Object.keys(components), function (key, value) {
            var option = document.createElement("option");
            option.value = value;
            option.text = value;
            select.appendChild(option);
          ...
          break;
        case 2:
          $('#components').html('<h3>' + selected[0] + '</h3>');
          var select = document.createElement('select');
          select.id = 'component-1-select';
          select.setAttribute('class', 'form-control form-group');
          ...
          option.text = "Vyberte";
          select.appendChild(option);
          $.each(components[selected[0]]["components"], function (key, value) {
            var option = document.createElement("option");
            option.value = key;
        ...
        case 3:
          ...
          option.text = "Vyberte";
          select.appendChild(option);
          $.each(components[selected[0]]["components"][selected[1]]["components"], function (key, value) {
            var option = document.createElement("option");
            option.value = key;
            if (value instanceof Object) {
              option.text = key;
        ...
        case 4:
          ...
          option.text = "Vyberte";
          select.appendChild(option);
          $.each(components[selected[0]]["components"][selected[1]]["components"][selected[2]]["components"], function (key, value) {
            var option = document.createElement("option");
            option.value = key;
            if (value instanceof Object) {
              option.text = key;
...
function showCriteria() {
  var scales = components[selected[0]]['scales'];
  $('#scales').html('');
  $.each(scales, function (key, value) {
    $('#scales').append('<h4>' + key + '</h4>');
    if (value instanceof Object) {
      $.each(value, function (num, desc) {
        ...
        label.setAttribute("for", key + ' ' + num);
  ...
  button.type = 'button';
  button.id = 'new-deviation';
  button.setAttribute('class', 'btn btn-secondary btn-block btn-lg');
  button.value = 'Pridať ďalšiu poruchu v rámci komponentu';
  $('#scales').append(button);
  var button = document.createElement('input');
  button.type = 'button';
  button.id = 'new-component';
  button.setAttribute('class', 'btn btn-secondary btn-block btn-lg');
  button.value = 'Posudzovať ďalší komponent';
  $('#scales').append(button);
}
...
function recordDeviation() {
  ...
  var strengthText = '';
  $.each(scales, function (key, value) {
    var el = $('[name="' + key + '"]:checked');
    strength = strength + el.val();
    var id = el.attr('id');
    var label = $('[for="' + id + '"]').text();
    strengthText = strengthText + label + ' ';
  });
  var lastStep = substep - 1;
  var component = $('#component-' + lastStep + '-select').val();
  var componentText = $('#component-' + lastStep + '-select option[value=' + component + ']').text();
  var deviation = component + '.' + strength + ' - ' + componentText + ' - ' + strengthText;
  evaluated.push(deviation);
}

function refreshEvaluatedDeviations() {
  if (evaluated.length > 0) {
    $('.evaluated').html('<h3>Posúdené poruchy</h3>');
    $.each(evaluated, function (key, deviation) {
      $('.evaluated').append(deviation + '</br>');
    });
  }
}

function showPersonalInfo() {
  $('#personal-data').html('<h3>Identifikačné údaje</h3>');
  var person = '';
  if (!!$('#academic-degree').val()) {
    person += $('#academic-degree option:selected').text() + ' ';
  }
  person = person + $('#name').val() + ' ' + $('#surname').val();
  if (!!$('#academic-degree-after').val()) {
    person += ', ' + $('#academic-degree-after option:selected').text();
  }
  var birthDate = new Date($('#birth-date').val());
  person += ' (' + birthDate.toLocaleDateString('sk') + ', ' + $('#birth-place').val() + ')<br>';
  $('#personal-data').append(person);

  var address = '<strong>Trvalé bydlisko:</strong>&nbsp;';
  address += $('#street').val() + ' ' + $('#street-number').val() + ', ' + $('#zip').val() + ' ' + $('#city').val() + ', ' + $('#country option:selected').text() + '<br>';
  $('#personal-data').append(address);

  var contact = '<strong>Kontaktné údaje:</strong>&nbsp;' + $('#email').val();
  if (!!$('#phone').val()) {
    contact += ', ' + $('#phone').val();
  }
  if (!!$('#mobile').val()) {
  ...
  contact += '<br>';
  $('#personal-data').append(contact);

  var maritalStatus = '<strong>Rodinný stav:</strong>&nbsp;' + $('#marital-status option:selected').text()+ '<br>';
  var education = '<strong>Vzdelanie:</strong>&nbsp;' + $('#education option:selected').text()+ '<br>';
  var employmentStatus = '<strong>Zamestnanie:</strong>&nbsp;' + $('#employment-status option:selected').text()+ '<br>';
  var employmentType = '<strong>Pracovno-právny vzťah:</strong>&nbsp;' + $('#marital-type option:selected').text()+ '<br>';
  $('#personal-data').append(maritalStatus);
  $('#personal-data').append(education);
  $('#personal-data').append(employmentStatus);
  $('#personal-data').append(employmentType);

  var disability = '<strong>Priznaná invalidita:</strong>&nbsp;';
  disability += $('[for="' + $('[name="assigned_disability"]:checked').attr('id') + '"]').text();
  if($('[name="ztp_holder"]:checked').val() === 'yes') {
    disability += ', Držiteľ preukazu ZŤP';
  }
  if($('[name="ztps_holder"]:checked').val() === 'yes') {
    disability += ', Držiteľ preukazu ZŤP/S';
  }
  disability += '<br>';
  $('#personal-data').append(disability);

  if(!!$('#diagnosis_mkch10').val()) {
    var mkch10 = '<strong>Diagnóza podľa MKCH10:</strong>&nbsp;' + $('#diagnosis_mkch10').val()+ '<br>';
    $('#personal-data').append(mkch10);
  }

  if(!!$('#diagnosis_brief').val()) {
    var brief = '<strong>Stručná anamnéza:</strong>&nbsp;' + $('#diagnosis_brief').val()+ '<br>';
    $('#personal-data').append(brief);
  }
}
